...
def request_withdrawal():
    ...
    # If head, auto-approve and process
    if g.user.role == 'head':
        ...
        if success:
            ...
            # Notify user
            create_notification(withdrawal.user_id, "Withdrawal Processed", f"Your withdrawal of KES {amount} has been processed.")
            send_email_notification(g.user.email, "Withdrawal Processed", f"Your withdrawal of KES {amount} has been completed.")
        ...
        # Non-head: notify all heads
        heads = User.query.filter_by(role='head').all()
        for head in heads:
            if head.id != g.user.id:
                create_notification(head.id, "New Withdrawal Request", f"{g.user.name} requested a withdrawal of KES {amount}.")
                send_email_notification(head.email, "New Withdrawal Request", f"{g.user.name} requested a withdrawal of KES {amount}. Please approve.")
...
def approve_withdrawal(withdrawal_id):
    ...
    if len(approvers) >= 2:
        ...
        if success:
            ...
            # Notify requester
            create_notification(withdrawal.user_id, "Withdrawal Approved", f"Your withdrawal of KES {withdrawal.amount} has been approved and processed.")
            send_email_notification(withdrawal.user.email, "Withdrawal Approved", f"Your withdrawal of KES {withdrawal.amount} has been approved.")
        ...
        # Notify requester that one head approved
        create_notification(withdrawal.user_id, "Withdrawal Approval Progress", f"One head has approved your withdrawal. Need {2 - len(approvers)} more head(s).")
...
def reject_withdrawal(withdrawal_id):
    ...
    # Notify requester
    create_notification(withdrawal.user_id, "Withdrawal Rejected", f"Your withdrawal of KES {withdrawal.amount} was rejected.")
    send_email_notification(withdrawal.user.email, "Withdrawal Rejected", f"Your withdrawal of KES {withdrawal.amount} was rejected.")
